...
void testThreadSafe(Tests& tests);
void testThreadSafe_runner(
	std::recursive_mutex& the_mutex,
	MyDetector& detector,
	std::vector<std::string>& strings,
	size_t& string_index,
	std::vector<bool>& strings_results
);
...
{
	//
	typedef MyStack<int> Stack;
	typedef MyQueue<int> Q;
	typedef MyDetector D;
	typedef int T;
...
{
	//
	auto t = tests.createTest("Thread Safe");
	...
	t->assertNoException(
		[t]()
		{
			...
			//	Build a pool of palindromes
			t->log("Building pool of palindromes");
			for ( size_t i = 0; i < 1000; i++ ) {
				
				auto p = rand.getString(10000, true);
				auto p_rev = p;
				std::reverse(p_rev.begin(), p_rev.end());
				p += p_rev;
				
				strings.push_back( p );
				strings_results.push_back(false);
				std::cout << "." << std::flush;
			}
			std::cout << std::endl;
			
			//	Launch all threads
			t->log("Launching all threads");
			for ( size_t thread_index = 0; thread_index < 20; thread_index++ ) {
				
				auto t = std::unique_ptr<std::thread>
				(
					new std::thread
					(
						&testThreadSafe_runner,
						std::ref(the_mutex),
						std::ref(detector),
						std::ref(strings),
						std::ref(string_index),
						std::ref(strings_results)
					)
				);
			...
			//	Join all threads
			t->log("Joining all threads");
			for ( auto& t : threads ) {
				t->join();
			}
			threads.clear();
			
			//	Verify all palindromes were detected correctly
			//	(hoping to spot race conditions)
			for ( size_t i = 0; i < strings_results.size(); i++ ) {
				
				bool result = strings_results[i];
				
				if ( result == false ) {
					t->assertTrue
					(
						result, 1,
						"Race condition probably prevented palindrome detection at index=" + std::to_string(i)
					);
				}
			}
			t->assertTrue(true, 1, "Palindrome detection is probably thread-safe.");
...
{
	//
	std::unique_lock lock(the_mutex, std::defer_lock);
	
	//
	while (true)
	{
		//	Grab next string, or quit if done
		lock.lock();
		if ( string_index >= strings.size() ) {
			break;
		}
		auto iteration_index = string_index;
		auto s = strings[ iteration_index ];
		string_index++;
		lock.unlock();
		
		//
		// std::cout << "Thread #" << std::this_thread::get_id() << ": Begin" << std::endl;
		bool result = detector.isPalindrome(s);
		strings_results[ iteration_index ] = result;
		// std::cout << "Thread #" << std::this_thread::get_id() << ": End ==> " << (result ? "True" : "False") << std::endl;
	}
}
